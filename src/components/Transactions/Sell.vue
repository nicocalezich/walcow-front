     ...
     <vue-form :state="formState" @submit.prevent="send()">
       <br>
        ...
        <div v-else class="amount-bought">
          <label>You will sell <b>{{this.formData.amount}}</b> {{this.selectedCrypto}} for <b>${{calculateSell}}</b> USD </label>
        </div>

        <div v-if="!sellSuccess">
          <button :disabled="formState.$invalid || this.formData.amount <= 0 || insufficientBalance" type="submit" class="btn btn-light confirm">Confirm sale</button>
        </div> 
        <div v-else>
          <div class="alert alert-success" role="alert">   
            successful sale <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-circle" viewBox="0 0 16 16">
